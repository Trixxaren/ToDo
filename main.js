...
class TaskManager {
  ...
  addNewTask(description) {
    ...
    if (description.trim()) {
      // trim tar bort alla onödiga mellanslag
  ...
  renderActiveTaskList() {
    ...
    if (this.activeTasks.length === 0) {
      ...
      this.activeTasks.forEach((task, index) => {
        // Går igenom varje aktiv todo och visar den
  ...
  renderCompletedTaskList() {
    ...
    const completedTaskMessage = document.getElementById(
      "completedTaskMessage"
    );
    // Inga färdiga todos
    if (this.completedTasks.length === 0) {
      completedTaskMessage.innerText =
        "Inga färdiga ToDos, börja jobba förfan!";
    } else {
      completedTaskMessage.innerText = "";
      this.completedTasks.forEach((task, index) => {
        // Går igenom varje färdig todo och visar
  ...
  editTask(index, status) {
    const taskList =
      status === "active" ? this.activeTasks : this.completedTasks; // väljer vilken lista aktiva/färdiga
    const taskDivs =
      status === "active"
        ? document
            .getElementById("activeTaskList")
            .getElementsByClassName("task")
        : document
            .getElementById("completedTaskList")
            .getElementsByClassName("task");
    const taskDiv = taskDivs[index]; // Hämtar diven för todon
    this.createEditInputField(
      taskDiv,
      index,
      status,
      taskList[index].description
    ); // Skapar input för att ändra
  }
